...
from utils.classes.ui import Button, Text
...
class GameRenderer:
        ...
        if start[0] == "T":
            ...
            # From Tableau to Foundation
                ...
                if end[1] == "A":
                    ...
                    moved = False
                    for i in range(len(self.game.foundation)):
                        if self.game.foundation[i].add_card(self.game.tableau.tableau[startCol][-1]):
                            self.game.foundation[i].pile[-1].deselect()
                            # Successful move
                            self.game.tableau.fetch_single_card(startCol)
                            self.moves += 1
                            moved = True
                            break
                    if not moved:
        ...
        # WASTE
            ...
            # From Waste to Foundation
                ...
                if end[1] == "A":
                    ...
                    moved = False
                    for i in range(len(self.game.foundation)):
                        if self.game.foundation[i].add_card(self.game.stock.waste[-1]):
                            self.game.foundation[i].pile[-1].deselect()
                            # Successful move
                            self.game.stock.fetch_top_card()
                            self.moves += 1
    ...
    def auto_move(self):
        ...
        if self.selected_card_location[0] == "W":
            ...
            if not moved:
                for i in range(len(self.game.tableau.tableau)):
                    if self.game.tableau.add_single_card(self.game.stock.waste[-1], i):
                        self.game.tableau.tableau[i][-1].face_up()
                        self.game.stock.fetch_top_card()
                        self.moves += 1
                        moved = True
                        break
            ...
            cardIdx = int(self.selected_card_location[2:])
            self.game.tableau.tableau[colIdx][cardIdx].deselect()
            if cardIdx == len(self.game.tableau.tableau[colIdx]) - 1:
                # Single Card Move
                self.game.tableau.tableau[colIdx][cardIdx].deselect()
                for i in range(len(self.game.foundation)):
                        if self.game.foundation[i].add_card(self.game.tableau.tableau[colIdx][-1]):
                            self.game.tableau.fetch_single_card(colIdx)
                            self.moves += 1
                            moved = True
                            break
                ...
                if not moved:
                    for i in range(len(self.game.tableau.tableau)):
                        if i != colIdx and self.game.tableau.add_single_card(self.game.tableau.tableau[colIdx][-1], i):
                            self.game.tableau.fetch_single_card(colIdx)
                            self.moves += 1
                            moved = True
                            break
                ...
                # Multiple Card Move
                cards = self.game.tableau.tableau[colIdx][cardIdx:]
                for card in cards:
                    card.deselect()
                for i in range(len(self.game.tableau.tableau)):
                    if self.game.tableau.add_multiple_cards(cards, i):
                        # Successful move
                        if len(self.game.tableau.tableau[colIdx]) > 0:
                            self.game.tableau.tableau[colIdx][-1].face_up()
                        if self.game.tableau.fetch_multiple_cards(colIdx, len(self.game.tableau.tableau[colIdx]) - cardIdx) == None:
                            # raise Exception("Card is None after fetching")
            ...
            colIdx = int(self.selected_card_location[1:])
            self.game.foundation[colIdx].pile[-1].deselect()
            for i in range(len(self.game.tableau.tableau)):
                if self.game.tableau.add_single_card(self.game.foundation[colIdx].pile[-1], i):
                    self.game.foundation[colIdx].pop_card()
                    self.moves += 1
                    moved = True
                    break
    ...
    def store_state(self):
        # self.states.append(self.game.backup())
