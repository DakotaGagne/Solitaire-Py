...
class Spider_Deck:
    ...
    def __init__(self, suitCnt = 1):
        ...
        if suitCnt == 4:
            self.cards = list(Card(suit, rank) for suit in self.SUITS * 2 for rank in self.RANKS)
        elif suitCnt == 2:
            self.cards = list(Card(suit, rank) for suit in [self.SUITS[0], self.SUITS[3]] * 4 for rank in self.RANKS)
        elif suitCnt == 1:
            self.cards = list(Card(suit, rank) for suit in [self.SUITS[3]] * 8 for rank in self.RANKS)
